...
export function getPopularityRank(tricks: Trick[], trickId: string) {
  ...
  const favouriteOrder = [...tricks].sort(
    (a, b) => b.favouriteCount - a.favouriteCount,
  );
  ...
  return (
    [...tricks]
      .sort((a, b) => {
        const aScore =
          viewOrder.findIndex((item) => item.id === a.id) +
          favouriteOrder.findIndex((item) => item.id === a.id);
        const bScore =
          viewOrder.findIndex((item) => item.id === b.id) +
          favouriteOrder.findIndex((item) => item.id === b.id);
...
function relatedTrickScore(source: Trick, candidate: Trick) {
  ...
  const sharedTerms = [...trickTerms(candidate.name)].filter((term) =>
    sourceTerms.has(term),
  );
  const termScore = sharedTerms.reduce(
    (score, term) => score + Math.min(term.length, 8),
    0,
  );
  const categoryScore = candidate.category === source.category ? 3 : 0;
  const contextScore =
    candidate.context && candidate.context === source.context ? 1 : 0;
