...
import {
  setTrickFavourite,
  useFavouriteTrickSlugs,
} from "@/hooks/use-favourite-tricks";
...
export function TrickViewer({
  ...
  const [metricAdjustments, setMetricAdjustments] = useState<
    Record<string, { views: number; favourites: number }>
  >({});
  ...
  useEffect(() => {
    ...
    console.info("[trick-metrics] Recording view", {
      trickId: trick.id,
      slug: trick.slug,
    });
    void createClient()
      .rpc("record_trick_view", { target_trick_id: trick.id })
      .then(({ error }) => {
        if (error) {
          viewedThisSession.delete(trick.id);
          console.error("[trick-metrics] View failed", {
            trickId: trick.id,
        ...
        console.info("[trick-metrics] View recorded", {
          trickId: trick.id,
          slug: trick.slug,
        });
      });
  ...
  const rankedTricks = useMemo(
    () =>
      tricks.map((candidate) => ({
        ...candidate,
        viewCount:
          candidate.viewCount + (metricAdjustments[candidate.id]?.views ?? 0),
        favouriteCount:
          candidate.favouriteCount +
          (metricAdjustments[candidate.id]?.favourites ?? 0),
      })),
  ...
  const currentMetrics =
    rankedTricks.find((candidate) => candidate.id === trick.id) ?? trick;
  ...
  const toggleFavourite = () => {
    ...
    setMetricAdjustments((current) => ({
      ...current,
      [trick.id]: {
        views: current[trick.id]?.views ?? 0,
        favourites: (current[trick.id]?.favourites ?? 0) + amount,
      },
    }));
    console.info("[trick-metrics] Updating favourite", {
      trickId: trick.id,
      slug: trick.slug,
      amount,
    });
    void createClient()
      .rpc("change_trick_favourite", { target_trick_id: trick.id, amount })
      .then(({ error }) => {
        if (error)
          console.error("[trick-metrics] Favourite update failed", {
            trickId: trick.id,
            message: error.message,
            code: error.code,
            details: error.details,
          ...
          console.info("[trick-metrics] Favourite updated", {
            trickId: trick.id,
            slug: trick.slug,
            amount,
          });
  ...
  return (
    <article className={styles.viewer} aria-live="polite">
      <TrickViewerHero
        trick={trick}
        category={category}
        controllerPlatform={controllerPlatform}
      ...
      <TrickViewerContent
        ...
        controllerPlatform={controllerPlatform}
        stance={stance}
        onControllerPlatformChange={onControllerPlatformChange}
        onStanceChange={onStanceChange}
      />
      <TrickViewerNavigation
        previousTrick={previousTrick}
        nextTrick={nextTrick}
      />
      <RelatedTricks
        relatedTricks={findRelatedTricks(rankedTricks, trick).map(
          ({ trick: related }) => related,
        )}
