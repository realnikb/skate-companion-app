...
const archiveSeasons = [
  {
    year: "2026",
    label: "Season 4",
    detail: "X Games Arrives",
    image: "/images/home/seasons/season-4.png",
    href: "/seasons/season-4",
  },
  {
    year: "2025",
    label: "Season 3",
    detail: "The Isle of Grom",
...
export function HomePage({
  ...
  const { slugs: sessionLineSlugs, save: saveSessionLine } =
    useSessionLine(tricks);
  ...
  const pickerResults = useMemo(() => {
    const availableTricks = tricks.filter(
      (trick) =>
        isSessionTrick(trick) && !sessionLineSlugs.includes(trick.slug),
    );
    if (!pickerQuery.trim()) return availableTricks.slice(0, 6);
    return availableTricks
      .filter((trick) => matchesTrickSearch(trick, pickerQuery))
      .slice(0, 8);
      ...
      [...tricks]
        .sort(
          (a, b) =>
            b.viewCount +
              b.favouriteCount * 5 -
              (a.viewCount + a.favouriteCount * 5) || a.sortOrder - b.sortOrder,
        )
        .slice(0, 5),
    [tricks],
  );
  const continueTricks = useMemo(
    () =>
      trickHistory
        .slice(0, 4)
        .map((entry) =>
          tricks.find((candidate) => candidate.slug === entry.slug)!,
        ),
    [trickHistory, tricks],
  );
  const favouriteTricks = useMemo(
    () =>
      [...favouriteTrickSlugs]
        .reverse()
  ...
  return (
    <main className={styles.homeShell}>
      {configError && (
        <section className={styles.configError} role="alert">
          <p className={styles.kicker}>Configuration</p>
          <h2>Supabase environment missing</h2>
          <p>{configError}</p>
        </section>
      )}
              ...
              <button
                aria-label={
                  homeLineSearch.isComplete
                    ? "Build this trick line"
                    : "View trick search results"
                }
                type="submit"
                  ...
                  <span>
                    {homeLineSearch.isLineQuery
                      ...
                      : homeSearchResults.length
                        ? "Add a trick to your line"
                        : "No matching tricks"}
                  </span>
                  <button
                    aria-label="Close search results"
                    onClick={() => setIsHomeSearchOpen(false)}
                  >
                    <X />
                  </button>
                </div>
                {homeLineSearch.isLineQuery ? (
                  <div className={styles.lineSearchResult}>
                      ...
                      {homeLineSearch.segments.map((segment, index) => {
                        ...
                        return (
                          <span
                            ...
                            className={
                              trick
                                ? styles.resolvedLineTrick
                                : styles.unresolvedLineTrick
                            }
                          >
                            {trick?.name ?? segment}
                          </span>
                    ...
                    {homeLineSearch.segments.length > 3 ? (
                      ...
                      <p>
                        We couldn’t find: {homeLineSearch.unresolved.join(", ")}
                        . Try another name or alias.
                      </p>
                    ) : homeLineSearch.isComplete ? (
                      <button type="button" onClick={buildSearchedLine}>
                        Build this line <ArrowUpRight />
                      </button>
                    ) : (
                      <p>
                        Add at least two different tricks, joined with “to”.
                      </p>
                    )}
                  </div>
                ) : (
                    ...
                    return (
                      ...
                      >
                        <span>
                          ...
                          <small>{trick.context ?? trick.category}</small>
                        </span>
                        <span className={isPinned ? styles.pinnedResult : ""}>
                          {isPinned ? <Check /> : <Plus />}
                          {isPinned ? "Pinned" : isFull ? "Line full" : "Add"}
                        </span>
                      </button>
                    );
                  })
      ...
      <section className={styles.sessionSection} id="session-line">
        <div className={styles.sectionHeading}>
          <div>
            <ListPlus />
            <h2>Current session line</h2>
            <span>{sessionLine.length} / 3 tricks</span>
          </div>
          {sessionLine.length > 0 && (
            <button onClick={() => saveSessionLine([])}>
              Clear line <X />
            </button>
          )}
        </div>
        ...
        {sessionLine.length === 0 ? (
          <button className={styles.emptySession} onClick={openTrickPicker}>
            ...
            <small>
              Search above and pin up to three tricks in the order you want to
              perform them.
            </small>
          </button>
        ) : (
          <div className={styles.sessionLine}>
            {sessionLine.map((trick, index) => (
              <article
                className={styles.sessionTrick}
                key={trick.slug}
                style={getCategoryTheme(
                  categories.find(
                    (category) => category.slug === trick.category,
                  ) ?? trick.category,
                )}
              >
                <div className={styles.sessionTrickHeader}>
                  ...
                  <div className={styles.sessionActions}>
                    <button
                      aria-label={`Move ${trick.name} earlier`}
                      disabled={index === 0}
                      onClick={() => moveSessionTrick(index, -1)}
                    >
                      <ArrowUp />
                    </button>
                    <button
                      aria-label={`Move ${trick.name} later`}
                      disabled={index === sessionLine.length - 1}
                      onClick={() => moveSessionTrick(index, 1)}
                    >
                      <ArrowDown />
                    </button>
                    <button
                      aria-label={`Remove ${trick.name}`}
                      onClick={() =>
                        saveSessionLine(
                          sessionLineSlugs.filter(
                            (slug) => slug !== trick.slug,
                          ),
                        )
                      }
                    >
                ...
                <button
                  ...
                  onClick={() => openLearning(trick.slug)}
                  aria-label={`Watch ${trick.name} preview`}
                >
                  {trick.posterUrl ? (
                    <Image
                      src={trick.posterUrl}
                    ...
                    <span className={styles.sessionPreviewFallback}>
                      <Play />
                      <small>Preview available in guide</small>
                    </span>
                  )}
                  <span className={styles.sessionPreviewLabel}>
                    <Play fill="currentColor" /> Preview
                  </span>
                </button>
                ...
                <button
                  className={styles.sessionCommand}
                  onClick={() => openLearning(trick.slug)}
                  aria-label={`View full controls for ${trick.name}`}
                >
                  {hasControls(trick.controls) ? (
                    <ControlSequence
                      variants={controlsForStance(trick.controls, stance)}
            ...
            {sessionLine.length < 3 && (
              <button
                className={styles.addSessionTrick}
                onClick={openTrickPicker}
              >
                <Plus />
                <span>
                  Add trick{" "}
                  <small>
                    {3 - sessionLine.length} slot
                    {3 - sessionLine.length === 1 ? "" : "s"} left
                  </small>
                </span>
              </button>
            )}
      ...
      {favouriteTricks.length > 0 && (
        <section className={styles.contentSection}>
          <div className={styles.sectionHeading}>
            ...
            <button onClick={() => router.push("/tricks")}>
              Browse tricks <ArrowUpRight />
            </button>
          </div>
          <div className={styles.continueGrid}>
            {favouriteTricks.map((trick) => {
              const trickCategory = categories.find(
                (item) => item.slug === trick.category,
              );
              return (
                <button
                  className={styles.continueCard}
                  key={trick.id}
                  style={getCategoryTheme(trickCategory ?? trick.category)}
                  onClick={() => openLearning(trick.slug)}
                >
                  <span className={styles.continueImage}>
                    {(trick.posterUrl ?? trick.originalPosterUrl) ? (
                      <Image
                        src={(trick.posterUrl ?? trick.originalPosterUrl)!}
                        alt={`${trick.name} trick preview`}
                        fill
                        sizes="(max-width: 800px) 76vw, 25vw"
                      />
                    ) : (
                    ...
                    <span className={styles.favouriteBadge}>
                      <Star fill="currentColor" /> Favourite
                    </span>
                  </span>
      ...
      <section className={styles.contentSection}>
        ...
        {continueTricks.length ? (
          ...
          <button
            className={styles.emptyContinue}
            onClick={() => router.push("/tricks")}
          >
            <BookOpen />
            <span>
              <strong>Start learning a trick</strong>
              <small>
                Your recently viewed guides and progress will appear here.
              </small>
            </span>
            <ArrowUpRight />
          </button>
        ...
        <div className={styles.featureGrid}>
          {popularTricks.map((trick, index) => (
            <button
              ...
              key={trick.id}
              style={getCategoryTheme(
                categories.find((item) => item.slug === trick.category) ??
                  trick.category,
              )}
              onClick={() => openLearning(trick.slug)}
            >
              <span
                ...
                aria-label={`Popularity rank ${index + 1}`}
              >
                {index + 1}
              </span>
              {trick.posterUrl ? (
              ...
              <span className={styles.cardCopy}>
                <strong>{trick.name}</strong>
                <small>
                  {trick.context ?? trick.category.replaceAll("-", " ")}
                </small>
              </span>
            </button>
          ))}
      ...
      <section className={styles.splitSection}>
          ...
          <div className={styles.tutorialFeature}>
            ...
            <span className={styles.tutorialCopy}>
              ...
              <span className={styles.tutorialPrompts}>
                ...
                <span>
                  Can&apos;t quite get a difficult trick? We&apos;ll show you
                  how.
                </span>
              </span>
            </span>
        ...
        <div className={styles.recentPanel}>
          ...
          {[
            "Prewinds",
            "Landing your first 1080",
            "Mastering hardflps",
            "Using The Video/Photo Editor",
          ].map((name, index) => {
            ...
            return (
              <button
                ...
                onClick={() => trick && openLearning(trick.slug)}
              >
      ...
      <section className={styles.newsSection} id="news">
        <div className={styles.newsLead}>
          ...
          <p>
            Block Parties returns for another weekend, X Games San Van rolls
            into week four, and the team shares what it is fixing next.
          </p>
          <Link href="/news/the-weekly-grind-aug-11-2026">
            Read the story <ArrowUpRight />
          </Link>
        </div>
        <Link
          className={styles.newsImage}
          href="/news/the-weekly-grind-aug-11-2026"
          aria-label="Read The Weekly Grind for August 11, 2026"
        >
          <Image
      ...
      <Dialog open={isTrickPickerOpen} onOpenChange={setIsTrickPickerOpen}>
        <DialogContent className={styles.trickPicker} showCloseButton={false}>
          <div className={styles.pickerHeader}>
            <div>
              <span className={styles.pickerEyebrow}>
                Session line · {sessionLine.length} / 3
              </span>
              <DialogTitle className={styles.pickerTitle}>
                Add a trick
              </DialogTitle>
              <DialogDescription className={styles.pickerDescription}>
                Find the next trick in your line.
              </DialogDescription>
            </div>
            <button
              className={styles.pickerClose}
              aria-label="Close trick picker"
              onClick={() => setIsTrickPickerOpen(false)}
            >
              <X />
            </button>
          </div>
          ...
          <div className={styles.aliasTip}>
            <Lightbulb />
            <p>
              <strong>Did you know?</strong> You can search using alternate
              names—try “tre flip” for a 360 Flip.
            </p>
          </div>

          <div className={styles.pickerResults} aria-live="polite">
            <span className={styles.pickerResultsLabel}>
              {pickerQuery.trim()
                ? `${pickerResults.length} matches`
                : "Suggested tricks"}
            </span>
            {pickerResults.length > 0 ? (
              pickerResults.map((trick) => (
                <button
                ...
                >
                  <span className={styles.pickerResultIcon}>
                    <Gamepad2 />
                  </span>
                  <span className={styles.pickerResultCopy}>
                    <strong>{trick.name}</strong>
                    <small>
                      {trick.context ?? trick.category}
                      {trick.aliases[0]
                        ? ` · Also known as ${trick.aliases[0]}`
                        : ""}
                    </small>
                  </span>
          ...
          <div className={styles.pickerFooter}>
            <span>
              {3 - sessionLine.length} slot
              {3 - sessionLine.length === 1 ? "" : "s"} remaining
            </span>
            <button onClick={() => setIsTrickPickerOpen(false)}>
              {sessionLine.length ? "Done" : "Cancel"}
            </button>
          </div>
        </DialogContent>
      </Dialog>
