...
export function PostTagPicker({ options }: { options: PostTagOption[] }) {
  ...
  const matches = useMemo(() => {
    ...
    return options
      .filter(
        (option) =>
          !selected.some(
            (item) => item.kind === option.kind && item.id === option.id,
          ) &&
          (!term ||
  ...
  const remove = (target: PostTagOption) =>
    setSelected((items) =>
      items.filter(
  ...
  return (
    <div className={styles.tagPicker}>
      <button
        ...
        data-active={open || selected.length > 0}
      >
        <Tag />
        Tag skaters or crews
        ...
        <div className={styles.selectedTags}>
          {selected.map((item) => (
            <span key={`${item.kind}:${item.id}`}>
              {item.kind === "crew" ? <Users /> : <span>@</span>}
              {item.name}
              <button
                type="button"
                onClick={() => remove(item)}
              ...
              <input
                type="hidden"
                name={
                  item.kind === "crew" ? "tagged_crew_ids" : "tagged_user_ids"
                }
                value={item.id}
              />
            </span>
        ...
        <div className={styles.tagMenu}>
          <label>
            ...
            <input
              ...
              placeholder="Search skaters and crews..."
              autoFocus
            />
          </label>
          <div>
            {matches.map((option) => (
              ...
              >
                <i>{option.kind === "crew" ? <Users /> : "@"}</i>
                <span>
                  <strong>{option.name}</strong>
                  <small>
                    {option.kind === "crew" ? "Crew" : `@${option.handle}`}
                  </small>
                </span>
              </button>
            ))}
            {!matches.length && <p>No matches found.</p>}
          </div>
        </div>
      )}
    </div>
