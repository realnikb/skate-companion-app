...
export function CrewDirectory({crews,posts:initialPosts,hasMorePosts:initialHasMore,maps,tagOptions,viewer}:{crews:Crew[];posts:SocialPost[];hasMorePosts:boolean;maps:PostMapOption[];tagOptions:PostTagOption[];viewer?:{name:string;avatarUrl?:string;ownedCrews:{id:string;name:string}[]}}){
...
<div className={styles.socialGrid}><div className={styles.feed}><div id="social-composer">{viewer?<PostComposer playerName={viewer.name} avatarUrl={viewer.avatarUrl} ownedCrews={viewer.ownedCrews} maps={maps} tagOptions={tagOptions}/>:<PostComposer signedIn={false} maps={maps} tagOptions={tagOptions}/>}</div>{tab==="following"&&<p className={styles.feedContext}>Posts from the people and crews you follow</p>}{visible.map(post=><PostCard post={post} signedIn={Boolean(viewer)} key={post.id}/>)}{!visible.length&&!hasMore&&<div className={styles.emptyFeed}><strong>Nothing here yet.</strong><span>{tab==="crews"?"Crew posts will appear here as they’re published.":"Follow skaters and crews to shape your feed."}</span></div>}<div className={styles.feedLoader} ref={loadSentinel} aria-live="polite">{loading&&<><i aria-hidden="true"/><span>Loading more posts…</span></>}</div></div><aside className={styles.rail}><section><div className={styles.railHeading}><h3>Crews to watch</h3><Link href="/social/crews">Discover</Link></div><div className={styles.suggestions}>{crews.slice(0,4).map(crew=><Link href={`/social/crew/${crew.slug}`} key={crew.id}><CrewLogo initials={crew.initials} accent={crew.accent} imageUrl={crew.logoUrl} size="small"/><span><strong>{crew.name}</strong><small>{crew.memberCount} members · {crew.languages[0]?.flag}</small></span><i>{crew.recruitment==="recruiting"?"Join":"View"}</i></Link>)}</div></section></aside></div></section>
