  ...
  return (
    <form action={formAction} className={styles.orderEditor}>
      ...
      <div className={styles.orderHelp}>
        <div>
          <strong>Frontend category order</strong>
          <span>
            Drag categories into position. This controls the filters and
            sections on the public Skatepedia.
          </span>
        </div>
        <span>{changed ? "Unsaved changes" : "Order is saved"}</span>
      </div>
      <ol className={styles.orderList}>
        {items.map((category, index) => (
          <li
            className={draggedId === category.id ? styles.draggingCategory : ""}
            draggable
            key={category.id}
            onDragStart={(event) => {
          ...
          >
            ...
            <div className={styles.orderCategory}>
              <strong>
                {category.parentName && <em>{category.parentName} / </em>}
                {category.name}
              </strong>
              <span>
                /{category.slug} · {category.trickCount} direct tricks
              </span>
              <p>{category.description || "No category description yet."}</p>
            </div>
            <span
              className={`${styles.status} ${!category.isPublished ? styles.draft : ""}`}
            >
      ...
      <div className={styles.orderActions}>
        ...
        <button type="submit" disabled={pending || !changed}>
          {pending ? "Saving…" : "Save frontend order"}
        </button>
      </div>
    </form>
  );
