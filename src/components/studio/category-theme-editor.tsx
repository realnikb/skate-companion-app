...
export function CategoryThemeEditor({
  name,
  accentColor,
  gradientStartColor,
  gradientMiddleColor,
  gradientEndColor,
}: CategoryThemeEditorProps) {
    ...
    <section className={`${styles.panel} ${styles.colourPanel}`}>
      <div className={styles.colourHeading}>
        <div>
          ...
          <p>
            The accent identifies the category. Three colour stops create its
      ...
      <div
        ...
        style={{
          background: `linear-gradient(180deg, transparent 34%, rgba(8, 8, 11, .96) 100%), linear-gradient(135deg, ${start} 0%, ${middle} 52%, ${end} 100%)`,
        }}
      >
        <span>Preview</span>
        <strong>{name}</strong>
        <small style={{ color: accent }}>{name}</small>
      </div>
      <div className={styles.colourFields}>
        ...
        <label>
          ...
          />
          <code>{start.toUpperCase()}</code>
        </label>
        <label>
          <span>Gradient middle</span>
          <input
            type="color"
            name="gradient_middle_color"
            value={middle}
            onChange={(event) => setMiddle(event.target.value)}
          />
          <code>{middle.toUpperCase()}</code>
        </label>
        <label>
