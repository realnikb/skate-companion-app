...
export function MediaField({
  ...
  async function choosePoster(file: File) {
    ...
    try {
      const uploadedOriginal = await upload(
        file,
        "poster-original",
        safeExtension(file),
      );
      ...
      setStatus(
        error instanceof Error
          ? error.message
          : "Could not upload the original artwork.",
      );
    } finally {
      setUploading(false);
    }
  ...
  const chooser = (
    <label className={styles.uploadButton}>
      {uploading
        ...
        : kind === "poster"
          ? "Replace artwork"
          : `Choose ${accept === "video/*" ? "video" : "image"}`}
      <input
        type="file"
        accept={accept}
        disabled={uploading}
      ...
      <div className={`${styles.mediaField} ${styles.heroArtwork}`}>
        <div className={styles.heroArtworkHeader}>
          <div>
            ...
            <strong>Background + card thumbnail</strong>
            <p>
              Keep the full original for the page atmosphere, then frame a
              separate 16:9 card image.
            </p>
          </div>
          {chooser}
        </div>
        <div className={styles.posterVersions}>
          <div>
            <span>1 · Original background</span>
            <div className={styles.originalPreview}>
              {originalUrl ? (
                <img src={originalUrl} alt="Original poster preview" />
              ) : (
                <em>No original uploaded</em>
              )}
            </div>
          </div>
          <div>
            <span>2 · Card thumbnail</span>
            <div className={styles.thumbnailPreview}>
              {url ? (
                <img src={url} alt="Cropped thumbnail preview" />
              ) : (
                <em>No thumbnail created</em>
              )}
            </div>
          </div>
        </div>
        {posterFile && (
          <ImageCropper
            ...
            outputWidth={1280}
            outputHeight={720}
            label="Frame the card thumbnail"
            busy={uploading}
            onCancel={() => setPosterFile(undefined)}
            onCrop={uploadPosterCrop}
          />
        )}
        <input type="hidden" name={name} value={path} />
        {originalName && (
          <input type="hidden" name={originalName} value={originalPath} />
        )}
        {status && (
        ...
        <div className={styles.mediaActions}>
          {canRemove && path && (
            <button
              ...
              onClick={(event) => removeMedia(event.currentTarget)}
            >
              Remove image
            </button>
          )}
      ...
      <div className={styles.mediaPreview}>
        {url ? (
          accept === "video/*" ? (
            ...
            <img key={url} src={url} alt={`${label} preview`} />
          )
        ) : (
