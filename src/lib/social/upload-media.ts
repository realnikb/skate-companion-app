...
export type DirectUpload = { path: string; mediaType: "image" | "video"; mimeType: string; size: number };

export async function uploadSocialMedia(file: File, onProgress: (percentage: number) => void): Promise<DirectUpload> {
  const mediaType = file.type.startsWith("video/") ? "video" : "image";
  const limit = mediaType === "video" ? 100 * 1024 * 1024 : 10 * 1024 * 1024;
  if (!allowed.has(file.type) || file.size > limit) throw new Error("Use an image up to 10 MB or an MP4, WebM or MOV video up to 100 MB.");
  const supabase = createClient();
  const { data: { session } } = await supabase.auth.getSession();
  if (!session) throw new Error("Your session expired. Sign in and try again.");
  const extension = file.name.split(".").pop()?.toLowerCase().replace(/[^a-z0-9]/g, "") || (mediaType === "video" ? "mp4" : "webp");
  ...
  await new Promise<void>((resolve, reject) => {
    ...
    request.onload = () => request.status >= 200 && request.status < 300 ? resolve() : reject(new Error(JSON.parse(request.responseText || "{}").message || `Upload failed (${request.status}).`));
    request.send(file);
  });
  onProgress(100);
