...
export async function getCrews():Promise<Crew[]> {
    ...
    return rows.map(row=>{
        ...
        return {id:row.id,slug:row.slug,name:row.name,initials:row.name.split(/\s+/).slice(0,2).map(word=>word[0]).join("").toUpperCase(),tagline:row.tagline??"",description:row.description??"",location:row.location??"Worldwide",platform:row.platform??"Cross-platform",style:row.styles,languages:row.languages.map(languageInfo),recruitment:row.recruitment_status as RecruitmentStatus,memberCount:members.length,followerCount:"0",accent:crewAccent(row.slug),logoUrl:mediaUrl(row.logo_path),owner:{handle:owner?.handle??"owner",displayName:owner?.display_name??"Crew owner"},members,socials:links,discord:discord&&discordLink?{inviteUrl:discordLink.href,serverName:discord.guild_name,memberCount:discord.approximate_member_count,onlineCount:discord.approximate_online_count}:undefined,videos:(videosResult.data??[]).filter(video=>video.crew_id===row.id).map(video=>({title:video.title,type:video.video_type,duration:"",plays:"",color:crewAccent(row.slug)}))};
    });
}
