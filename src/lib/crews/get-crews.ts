...
export async function getCrews():Promise<Crew[]> {
    ...
    const ids=rows.map(row=>row.id),ownerIds=[...new Set(rows.map(row=>row.owner_id))];
    const [profilesResult,membersResult,linksResult,discordResult,videosResult]=await Promise.all([
        supabase.from("profiles").select("id,handle,display_name").in("id",ownerIds),
    ...
    return rows.map(row=>{
        ...
        return {id:row.id,slug:row.slug,name:row.name,initials:row.name.split(/\s+/).slice(0,2).map(word=>word[0]).join("").toUpperCase(),tagline:row.tagline??"",description:row.description??"",location:row.location??"Worldwide",platform:row.platform??"Cross-platform",style:row.styles,languages:row.languages.map(languageInfo),recruitment:row.recruitment_status as RecruitmentStatus,memberCount:members.length,followerCount:"0",accent:row.primary_color??crewAccent(row.slug),logoUrl:mediaUrl(row.logo_path),bannerUrl:mediaUrl(row.banner_path),ownerId:row.owner_id,owner:{handle:owner?.handle??"owner",displayName:owner?.display_name??"Crew owner"},members,socials:links,discord:discord&&discordLink?{inviteUrl:discordLink.href,serverName:discord.guild_name,memberCount:discord.approximate_member_count,onlineCount:discord.approximate_online_count}:undefined,videos:(videosResult.data??[]).filter(video=>video.crew_id===row.id).map(video=>({title:video.title,type:video.video_type,duration:"",plays:"",color:row.primary_color??crewAccent(row.slug)}))};
    });
}
