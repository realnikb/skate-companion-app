...
export async function getCategories(): Promise<TrickCategory[]> {
  ...
  const { data, error } = await supabase
    ...
    .select(
      "id, slug, name, description, page_eyebrow, page_heading, popular_heading, hero_image_path, accent_color, gradient_start_color, gradient_middle_color, gradient_end_color, parent_id, sort_order",
    )
