...
function findLineTrick(tricks: Trick[], query: string) {
  const normalizedQuery = normalizeSearchValue(query);
  const exactMatch = tricks.find((trick) =>
    searchableValues(trick).includes(normalizedQuery),
  );
...
export function parseTrickLine(
  query: string,
  tricks: Trick[],
  maximumLength = 3,
): TrickLineSearch {
  ...
  const resolved = segments.map((segment) => ({
    segment,
    trick: findLineTrick(tricks, segment),
  }));
  ...
  const uniqueTricks = matchedTricks.filter(
    (trick, index) =>
      matchedTricks.findIndex((candidate) => candidate.slug === trick.slug) ===
      index,
  );
