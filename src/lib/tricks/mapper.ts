...
import {
  normalizeTrickControls,
  parseTrickControls,
} from "@/lib/tricks/controls";
...
function mapControls(value: Json) {
  const controls =
    ...
    (Array.isArray(value) ? normalizeTrickControls(value) : null);
  if (!controls)
    throw new Error("Malformed trick controls returned from database.");
  return controls;
}
...
export function mapTrick(row: TrickRow): Trick {
  if (!row.trick_categories) {
    throw new Error(
      `Published trick is missing its category relation: ${row.slug}`,
    );
  }
