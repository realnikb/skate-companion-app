...
async function sendLink(_state: SignInState, formData: FormData, shouldCreateUser: boolean): Promise<SignInState> {
    void _state;
    ...
    if (!origin) {
        return { status: "error", message: "We couldn't start sign-in. Please try again." };
    }
...
export async function sendSignInLink(state: SignInState = initialState, formData: FormData) {
    return sendLink(state, formData, false);
}

export async function sendSignUpLink(state: SignInState = initialState, formData: FormData) {
