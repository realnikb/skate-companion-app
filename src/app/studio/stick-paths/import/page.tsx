...
export default async function ImportStickPathsPage() {
    ...
    return <main className={styles.content}><header className={styles.editorHeader}><div><Link href="/studio/stick-paths">← Back to stick paths</Link><h1>Import stick controls</h1></div></header><StickPathImporter existing={(data ?? []).map(mapStickPath)} /></main>;
}
