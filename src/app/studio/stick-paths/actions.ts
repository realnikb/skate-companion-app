...
export type StickPathActionState = {
...
export async function createStickPath(
  _state: StickPathActionState,
  formData: FormData,
): Promise<StickPathActionState> {
  const input = fields(formData);
  if (!input)
    return {
      status: "error",
      message: "Enter a name and at least one valid path point.",
    };
  const { supabase } = await requireStudioUser();
  let slug: string;
  try {
    slug = await availableSlug(supabase, input.name);
  } catch (error) {
    return {
      status: "error",
      message:
  ...
  if (value(formData, "intent") === "save-and-new")
    redirect("/studio/stick-paths/new");
  redirect(`/studio/stick-paths/${data.id}`);
}

export async function updateStickPath(
  _state: StickPathActionState,
  formData: FormData,
): Promise<StickPathActionState> {
  ...
  let slug: string;
  try {
    slug = await availableSlug(supabase, input.name, id);
  } catch (error) {
    return {
      status: "error",
      message:
