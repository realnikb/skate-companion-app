    ...
    <main className={styles.content}>
      <header className={styles.pageHeader}>
        <div>
          ...
          <h1>Stick paths</h1>
          <p>
            Draw each gesture once, then reuse it across any number of trick
            combinations.
          </p>
        </div>
        <div className={styles.headerActions}>
          <Link
            ...
            href="/studio/stick-paths/import"
          >
            Import JSON
          </Link>
          <Link className={styles.primaryButton} href="/studio/stick-paths/new">
            New stick path
          </Link>
        </div>
      </header>
      <div className={styles.categoryGrid}>
        {paths.map((path) => (
          <Link
            ...
            href={`/studio/stick-paths/${path.id}`}
            key={path.id}
          >
            <header>
              <div>
        ...
        {!paths.length && (
          <p className={styles.empty}>No reusable stick paths yet.</p>
        )}
      </div>
    </main>
  );
