...
export default async function EditCategoryPage({
  ...
  const [{ data: category, error }, { count }, { data: categories }] =
    await Promise.all([
    ...
    <main className={styles.content}>
      ...
      <StudioForm kind="category" isPublished={category.is_published}>
        ...
        <section className={`${styles.panel} ${styles.detailsPanel}`}>
          ...
          <div className={styles.field}>
            <label htmlFor="page_eyebrow">Hero eyebrow</label>
            <input
              id="page_eyebrow"
              name="page_eyebrow"
              defaultValue={
                category.page_eyebrow ?? "EA Skate Online Skatepedia"
              }
            />
          </div>
          <div className={styles.field}>
            <label htmlFor="page_heading">Hero heading</label>
            <input
              id="page_heading"
              name="page_heading"
              defaultValue={category.page_heading ?? "How to do {category}"}
            />
            <small>Use {"{category}"} to insert the category name.</small>
          </div>
          <div className={styles.field}>
            <label htmlFor="popular_heading">Popular tricks heading</label>
            <input
              id="popular_heading"
              name="popular_heading"
              defaultValue={category.popular_heading ?? "Popular {category}"}
            />
            <small>Use {"{category}"} to insert the category name.</small>
          </div>
        </section>
        <section className={`${styles.panel} ${styles.mediaPanel}`}>
          <div className={styles.sectionHeading}>
            ...
            <p>
              Set the background artwork used at the top of this category page.
            </p>
          </div>
          <MediaField
            trickId={category.id}
            ...
            label="Hero image"
            accept="image/*"
            defaultPath={category.hero_image_path}
          />
        </section>
        <CategoryThemeEditor
          name={category.name}
          accentColor={category.accent_color}
          gradientStartColor={category.gradient_start_color}
          gradientMiddleColor={category.gradient_middle_color}
          gradientEndColor={category.gradient_end_color}
        />
        <aside className={`${styles.panel} ${styles.publishingPanel}`}>
          ...
          <p className={styles.settingsNote}>
            {count ?? 0} tricks currently belong to this category.
          </p>
        </aside>
      </StudioForm>
    </main>
