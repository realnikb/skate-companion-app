...
export default async function StudioCategoriesPage() {
  ...
  if (error)
    throw new Error(`Unable to load Studio categories: ${error.message}`);
  const counts = new Map<string, number>();
  tricks?.forEach((trick) =>
    counts.set(trick.category_id, (counts.get(trick.category_id) ?? 0) + 1),
  );
    ...
    <main className={styles.content}>
      <header className={styles.pageHeader}>
        <div>
          ...
          <h1>Categories</h1>
          <p>
            {categories?.length ?? 0} categories organise the trick catalogue.
          </p>
        </div>
        <Link className={styles.primaryButton} href="/studio/categories/new">
          New category
        </Link>
      </header>
      <CategoryOrderEditor
        categories={(categories ?? []).map((category) => ({
          ...
          trickCount: counts.get(category.id) ?? 0,
