...
export type StudioActionState = { status: "idle" | "success" | "error"; message?: string; savedAt?: string; savedBy?: string };
...
const trickDifficulties = new Set(["beginner", "intermediate", "advanced", "expert"]);

function difficultyValue(formData: FormData) {
    const difficulty = value(formData, "difficulty");
    return trickDifficulties.has(difficulty) ? difficulty as "beginner" | "intermediate" | "advanced" | "expert" : null;
}

export async function createTrick(_state: StudioActionState, formData: FormData): Promise<StudioActionState> {
    const name = value(formData, "name");
    const slug = value(formData, "slug");
    const categoryId = value(formData, "category_id");
    const description = value(formData, "description");
    if (!name || !slug || !categoryId || !description) return { status: "error", message: "Complete every required field before creating the draft." };
    ...
    const { data: lastTrick } = await supabase.from("tricks").select("sort_order").order("sort_order", { ascending: false }).limit(1).maybeSingle();
    const { data: trick, error } = await supabase.from("tricks").insert({
        name, slug, category_id: categoryId, description, difficulty: difficultyValue(formData), aliases: [], controls: { version: 2, authoredStance: "goofy", variants: [] },
        video_path: "", controls_reference_path: "", controls_clean_path: "",
        is_published: false, needs_control_review: true, needs_description_review: true,
        sort_order: (lastTrick?.sort_order ?? 0) + 10, last_edited_by: user.email,
    }).select("id").single();
...
export async function createCategory(_state: StudioActionState, formData: FormData): Promise<StudioActionState> {
    const name = value(formData, "name");
    const slug = value(formData, "slug");
    const description = nullable(formData, "description");
    const parentId = nullable(formData, "parent_id");

    if (!name || !slug) return { status: "error", message: "Category name and slug are required." };
    if (!validSlug(slug)) return { status: "error", message: "Slug must contain lowercase letters, numbers and hyphens only." };

    const { supabase } = await requireStudioUser();
    if (parentId) {
        const { data: parent, error: parentError } = await supabase.from("trick_categories").select("id").eq("id", parentId).maybeSingle();
        if (parentError || !parent) return { status: "error", message: "Select a valid parent category." };
    }

    const { data: lastCategory, error: orderError } = await supabase.from("trick_categories").select("sort_order").order("sort_order", { ascending: false }).limit(1).maybeSingle();
    if (orderError) return { status: "error", message: orderError.message };

    const { data: category, error } = await supabase.from("trick_categories").insert({
        name,
        slug,
        description,
        parent_id: parentId,
...
export async function updateTrick(_state: StudioActionState, formData: FormData): Promise<StudioActionState> {
    ...
    if (!id || !name || !categoryId || !description || !videoPath) {
        return { status: "error", message: "Complete every required field before saving." };
    }
    if (!validSlug(slug)) return { status: "error", message: "Slug must contain lowercase letters, numbers and hyphens only." };
    ...
    try {
        ...
        return { status: "error", message: "Controls are invalid. Every step needs at least one valid input and every path point must stay inside the stick area." };
    }

    const aliases = value(formData, "aliases").split(/[\n,]/).map((alias) => alias.trim()).filter(Boolean);
    const { supabase, user } = await requireStudioUser();
    const pathIds = [...new Set(parsedControls.variants.flatMap((variant) => variant.steps.flatMap((step) => step.inputs.flatMap((input) => input.type === "stick" && input.pathId ? [input.pathId] : []))))];
    const { data: savedPaths, error: pathError } = pathIds.length
        ? await supabase.from("stick_paths").select("id,slug,name,points").in("id", pathIds)
        : { data: [], error: null };
    if (pathError) return { status: "error", message: `Could not validate stick paths: ${pathError.message}` };
    if ((savedPaths ?? []).length !== pathIds.length) return { status: "error", message: "One or more selected stick paths no longer exist. Choose another path and try again." };
    const pointsByPathId = new Map((savedPaths ?? []).map((row) => { const path = mapStickPath(row); return [path.id, path.points] as const; }));
    const hydratedControls = structuredClone(parsedControls);
    hydratedControls.variants.forEach((variant) => variant.steps.forEach((step) => step.inputs.forEach((input) => {
        if (input.type === "stick" && input.pathId) input.path.points = pointsByPathId.get(input.pathId) ?? input.path.points;
    })));
    const controls = hydratedControls as unknown as Json;
    const { data: savedTrick, error } = await supabase.from("tricks").update({
        name,
        slug,
    ...
    if (value(formData, "intent") === "save-and-next") {
        const { data: orderedTricks, error: nextError } = await supabase.from("tricks").select("id").order("name");
        if (nextError) return { status: "error", message: `Changes were saved, but the next trick could not be found: ${nextError.message}`, savedAt: savedTrick.updated_at, savedBy: savedTrick.last_edited_by ?? user.email };
        const currentIndex = (orderedTricks ?? []).findIndex((trick) => trick.id === id);
        const nextId = currentIndex >= 0 ? orderedTricks?.[currentIndex + 1]?.id : undefined;
        redirect(nextId ? `/studio/tricks/${nextId}` : "/studio/tricks");
    }
    return { status: "success", message: "All changes saved", savedAt: savedTrick.updated_at, savedBy: savedTrick.last_edited_by ?? user.email };
}

export async function updateCategory(_state: StudioActionState, formData: FormData): Promise<StudioActionState> {
    const id = value(formData, "id");
    const name = value(formData, "name");
    const slug = value(formData, "slug");
    ...
    if (!id || !name) return { status: "error", message: "Category name is required." };
    if (!validSlug(slug)) return { status: "error", message: "Slug must contain lowercase letters, numbers and hyphens only." };
    if (![accentColor, gradientStartColor, gradientMiddleColor, gradientEndColor].every(validHexColor)) return { status: "error", message: "Every category colour must be a six-digit hex colour." };

    const { supabase } = await requireStudioUser();
    const { error } = await supabase.from("trick_categories").update({
        name,
        slug,
...
export async function reorderCategories(_state: StudioActionState, formData: FormData): Promise<StudioActionState> {
    let ids: string[];
    try {
        const parsed: unknown = JSON.parse(value(formData, "category_ids"));
        if (!Array.isArray(parsed) || !parsed.every((id) => typeof id === "string") || new Set(parsed).size !== parsed.length) throw new Error();
        ids = parsed;
    } catch {
        return { status: "error", message: "The category order was invalid. Refresh and try again." };
    }
    const { supabase } = await requireStudioUser();
    const { data: current, error: readError } = await supabase.from("trick_categories").select("id");
    if (readError) return { status: "error", message: readError.message };
    if (ids.length !== current.length || current.some((category) => !ids.includes(category.id))) return { status: "error", message: "Categories changed while you were editing. Refresh and try again." };
    const results = await Promise.all(ids.map((id, index) => supabase.from("trick_categories").update({ sort_order: (index + 1) * 10 }).eq("id", id)));
