...
export const metadata: Metadata = {
  title: "Social | Skate Companion",
  description:
    "Share skate posts, discover crews and see what the community is doing.",
};
export const dynamic = "force-dynamic";
const SOCIAL_FEED_PAGE_SIZE = 10;

export default async function SocialPage() {
  const [crews, postPage, trendingPosts] = await Promise.all([
    getCrews(),
    getSocialPosts(0, SOCIAL_FEED_PAGE_SIZE + 1),
    getTrendingSocialPosts(10),
  ]);
  const posts = postPage.slice(0, SOCIAL_FEED_PAGE_SIZE);
  const hasMorePosts = postPage.length > SOCIAL_FEED_PAGE_SIZE;
  const supabase = await createClient();
  const [{ data: auth }, { data: mapRows }, { data: profileRows }] =
    await Promise.all([
  ...
  const maps: PostMapOption[] = (mapRows ?? []).map((map) => ({
    id: map.id,
    name: map.name,
    ...
    bounds: map.bounds as PostMapOption["bounds"],
  }));
