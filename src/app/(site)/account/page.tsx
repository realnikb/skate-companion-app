...
export default async function AccountPage() {
    ...
    const { data: profile } = userId ? await supabase.from("profiles").select("display_name,handle,avatar_path,preferred_controller,stance,updated_at").eq("id", userId).maybeSingle() : { data: null };
    ...
    return (
        <main className={styles.page}>
            ...
            <ProfileEditor displayName={profile?.display_name ?? email.split("@")[0] ?? "Skater"} email={email} avatarUrl={profile?.avatar_path ? `${process.env.NEXT_PUBLIC_SUPABASE_URL?.replace(/\/$/, "")}/storage/v1/object/public/profile-media/${profile.avatar_path}?v=${encodeURIComponent(profile.updated_at)}` : undefined} controller={profile?.preferred_controller ?? "xbox"} stance={profile?.stance ?? "regular"} />
            <section className={styles.grid}>
                <article className={styles.crewCard}><Users /><span>Crews</span><h2>{ownedCrews?.length ? "Crews you own" : "Your crew identity"}</h2>{ownedCrews?.length ? ownedCrews.map(crew => <Link key={crew.slug} href={`/social/${crew.slug}`}>{crew.name} · {crew.recruitment_status}</Link>) : <><p>Create your crew page and you’ll be shown publicly as its verified owner.</p><a href="#create-crew">Create your crew</a></>}</article>
