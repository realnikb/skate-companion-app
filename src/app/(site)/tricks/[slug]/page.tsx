...
function describeControl(control: NormalizedControlInput) {
  if (control.type === "stick")
    return `${control.action.replaceAll("-", " ")} the ${control.stick} stick along the shown path`;
  return `${control.action} ${control.control.replaceAll("-", " ")}`;
}

function categoryHeading(category: { name: string; pageHeading?: string }) {
  return (category.pageHeading || "How to do {category}").replaceAll(
    "{category}",
    category.name,
  );
}
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  if (category) {
    ...
    return {
      title,
      description,
      alternates: { canonical: `/tricks/${category.slug}` },
      openGraph: {
        title,
        description,
        type: "website",
        url: `/tricks/${category.slug}`,
      },
    };
  }
  try {
    ...
    return {
      title,
      description,
      alternates: { canonical: `/tricks/${trick.slug}` },
      openGraph: {
        title,
        description,
        type: "article",
        images: trick.posterUrl
          ? [
              {
                url: trick.posterUrl,
                alt: `${trick.name} demonstration in EA skate.`,
              },
            ]
          : [],
      },
      twitter: {
        ...
        title,
        description,
        images: trick.posterUrl ? [trick.posterUrl] : [],
      },
    };
  } catch {
    return { title: "Trick not found | Skate Companion" };
  }
}
...
export default async function TrickPage({ params }: Props) {
  ...
  if (landingCategory) {
    ...
    const structuredData = {
      ...
      "@type": "CollectionPage",
      name: `EA Skate ${landingCategory.name} trick guides`,
      description:
        landingCategory.description ??
        `Learn how to do ${landingCategory.name.toLowerCase()} in EA Skate.`,
      mainEntity: {
        "@type": "ItemList",
    ...
    const categoryNameIndex = heading
      .toLocaleLowerCase()
      .lastIndexOf(landingCategory.name.toLocaleLowerCase());
    const heroStyle = {
      ...getCategoryTheme(landingCategory),
      ...(landingCategory.heroImageUrl
        ? {
            backgroundImage: `linear-gradient(110deg,rgba(10,12,16,.88),rgba(10,12,16,.55)),url("${landingCategory.heroImageUrl.replaceAll('"', "%22")}")`,
          }
        : {}),
    };
    return (
      <main className={directoryPageStyles.main}>
        <script
        ...
        />
        <section className={directoryPageStyles.hero} style={heroStyle}>
          <div className={directoryPageStyles.heroInner}>
            <span>
              {landingCategory.pageEyebrow ?? "EA Skate Online Skatepedia"}
            </span>
            <h1>
              {categoryNameIndex >= 0 ? (
                <>
                  {heading.slice(0, categoryNameIndex)}
                  <br />
                  <em>{heading.slice(categoryNameIndex)}</em>
                </>
              ) : (
                heading
              )}
            ...
            <p>
              {landingCategory.description ??
                `Learn every ${landingCategory.name.toLowerCase()} in EA Skate with controller inputs, video demonstrations and focused trick guides.`}
            </p>
          </div>
        </section>
        <div className={directoryPageStyles.content}>
          <TrickDirectory
  ...
  let trick;
  try {
    trick = await getCachedTrick(slug);
  } catch {
    notFound();
  }
  const tricks = await getTricks();
  const category =
    categories.find((item) => item.slug === trick.category) ?? null;
  const article = /^[aeiou]/i.test(trick.name) ? "an" : "a";
  const structuredData = {
    "@context": "https://schema.org",
    "@type": "HowTo",
    ...
    step: hasControls(trick.controls)
      ? trick.controls.variants
          .flatMap((variant) => variant.steps)
          .map((step, index) => ({
            "@type": "HowToStep",
            position: index + 1,
            name: `Input ${index + 1}`,
            text: step.inputs
              .map(
                (input, inputIndex) =>
                  `${inputIndex ? (input.join === "or" ? "or " : "and ") : ""}${describeControl(input)}`,
              )
              .join(" "),
          }))
      : [
          {
            "@type": "HowToStep",
            position: 1,
            name: "Follow the Flick-It input",
            text: "Follow the controller input shown in the trick guide, then match the demonstration timing.",
          },
        ],
  };
  return (
